...
try {
  ...
  deployKeyInfoList.forEach(deployKeyInfo => {
    if (!deployKeyInfo.orgAndRepo || !deployKeyInfo.envName)
      argErrors.push(
        `Item ${i} has missing arg(s).  It must contain the orgAndRepo as well as the name of the environment variable containing the private ssh key.`
      );
    if (!process.env[`${deployKeyInfo.envName}`])
      argErrors.push(
        `The environment variable '${deployKeyInfo.envName}' for Item ${i} has not been populated.  It must be set under the 'env:' section of the step or the workflow.`
      );
    i++;
  });
